...
def forbiddenVelocity(velocity, obstacles, plan, step, startVelocity, rRad, oRad, timer):
    ...
    # iterate through all obstacles to find if need to slow down

    for i in range(n):
        # check that obstacle is active
        if obstacles[i].delay <= timer:
            # calculate directions of obstacles, assume linear
            # this is how we expect the obstacle's coordinates to change
            ...
            # find distance to intersection, and timesteps to reach assuming constant velocity for both
            rdist = np.abs(np.sqrt((plan[step][0] - intersect[0])**2 + (plan[step][1] - intersect[1])**2) - rRad -oRad)
            rtime = rdist/velocity
            odist = np.abs(np.sqrt((obstacles[i].x - intersect[0])**2 + (obstacles[i].y - intersect[1])**2) - oRad - rRad)
            otime = odist/obstacles[i].v

            # if roomba reaches it at the same time as the obstacle (up to 2 timesteps less)
            # possibly slow down. If roomba reaches before, don't slow down
            dist = np.sqrt((plan[step][0] - obstacles[i].x) ** 2 + (plan[step][1] - obstacles[i].y) ** 2)

            # emergency breaking takes priority
            # just make 10 error - for when really close, forbidden won't account for it because subtracting
            # oRad and rRad makes absolute distance larger
            if dist < rRad + oRad + 10:
                print("Slowing down for obstacle " + str(i))
                return (velocity - breaking)

            # find if intersect is in front of or behind roomba path. if behind, ignore
            # x' = x + vt, if t is negative, is behind
            t = (intersect[0] - plan[step][0])/velocity
            if t < 0:
                continue

            if 0 <= otime - rtime <= velocity:
                relativeVelocity = oDir - rDir
                rVelocity = np.sqrt(relativeVelocity[0]**2 + relativeVelocity[1]**2)
                # need to check how long it would take to make relative velocity 0.
                # If it is greater than timesteps to collision for otime, ignore
                stop = rVelocity/breaking
                if stop <= otime + rVelocity:
                    velocity -= breaking
                    # max sure doesn't exceed max
                    if np.abs(velocity) > startVelocity:
                        return -startVelocity
                    print("Slowing down for obstacle " + str(i))
                    return velocity
    # got through for loop without slowing
    velocity += breaking
...
def move(originalMap, plan, velocity, obstacles):
    ...
    with moviewriter.saving(fig, 'Roomba.gif', dpi=100):
        while flag:
            ...
            velocity = forbiddenVelocity(velocity, obstacles, plan, count, startVelocity, rRad, oRad, timer)
...
def main():
    ...
    if map[start[0]][start[1]] != 0 or 0 > start[0] or 0 > start[1] or dim < start[0] or dim < start[1]:
        print("Invalid start coordinate, please choose another area")
        return
    if map[end[0]][end[1]] != 0 or 0 > end[0] or 0 > end[1] or dim < end[0] or dim < end[1]:
        print("Invalid end coordinate, please choose another area")
        return
